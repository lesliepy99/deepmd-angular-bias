...
class DeepmdDataSystem:
    ...
    #  全量验证 相关辅助函数
    # ------------------------------------------------------------------
    def _infer_nframes(self, data_dict: dict) -> int:
        """猜测 data_dict 里共有多少帧（取第一个带 batch 维度的键）"""
        for k, v in data_dict.items():
            if k.startswith("find_") or k in ["natoms_vec", "default_mesh"]:
                continue
            if isinstance(v, np.ndarray):
                return v.shape[0]
        raise RuntimeError("无法推断验证数据帧数")

    def _slice_batch(self, data_dict: dict, idx) -> dict:
        """按照给定索引切出一个 batch（浅拷贝）"""
        sliced = {}
        for k, v in data_dict.items():
            if k in ["natoms_vec", "default_mesh"]:
                sliced[k] = v                      # 标量 / 常量直接复用
            elif isinstance(v, np.ndarray) and v.shape[0] >= len(idx):
                sliced[k] = v[idx]                # 带 batch 维度的张量
            else:
                sliced[k] = v                     # 其它对象原样放回
        return sliced
    ...
    #  对外接口：遍历 / 获取整个验证集
    # ------------------------------------------------------------------
    def iter_validation_batches(self, shuffle: bool = False):
        """generator：按各自 batch_size 顺序输出整个验证集"""
        for sys_idx in range(self.nsystems):
            full_test = self.get_test(sys_idx, n_test=-1)   # 所有验证帧
            nframes   = self._infer_nframes(full_test)
            print("system",sys_idx,"nframes",nframes)
            indices   = np.arange(nframes)
            if shuffle:
                np.random.shuffle(indices)

            bs = self.batch_size[sys_idx]
            for st in range(0, nframes, bs):
                idx_slice = indices[st : st + bs]
                yield self._slice_batch(full_test, idx_slice)

    def get_all_validation_batches(self, shuffle: bool = False) -> list[dict]:
        """一次性返回 *整个* 验证集切好的 batch 列表"""
        return list(self.iter_validation_batches(shuffle=shuffle))
