...
class DPTrainer:
    ...
    def _build_network(self, data, suffix="") -> None:
        ...
        self.place_holders["is_training"] = tf.placeholder_with_default(
        ...
        while cur_batch < stop_batch:
            ...
            if self.display_in_training and (cur_batch % self.disp_freq == 0):
                ...
                if self.run_opt.is_chief:
                    ...
                    valid_batches = (
                        valid_data.get_all_validation_batches()
                        if valid_data is not None else None
                    )
                    
                    # total = 0
                    # for b in valid_batches:
                    #     # 找到一个带 batch 维度的 ndarray 来读 shape[0]
                    #     for v in b.values():
                    #         if isinstance(v, np.ndarray) and v.ndim > 0:
                    #             total += v.shape[0]
                    #             break   # 该 batch 计数完毕，继续下一个 batch
                    # print("total",total)
                    # exit()
